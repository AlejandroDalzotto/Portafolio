  ...
  return (
    <section className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 justify-items-center w-full">
      {PROJECTS.map((project, idx) => {
        return (
          <article key={idx} className="ring-1 ring-white/[0.15] bg-gray-900 text-slate-50 w-72 h-96 shadow-lg m-4 p-4 rounded relative">
            <Image className="rounded-t" src={!project.img ? DefaultImage : project.img} alt={project.project_name} width={260} height={192} />
            <h3 className="font-semibold my-3 text-xl">{project.project_name}</h3>
